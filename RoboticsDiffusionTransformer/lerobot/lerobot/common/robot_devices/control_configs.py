...
from lerobot.lerobot.common.robot_devices.robots.configs import RobotConfig
from lerobot.lerobot.configs import parser
from lerobot.lerobot.configs.policies import PreTrainedConfig


# from common.robot_devices.robots.configs import RobotConfig
...
@ControlConfig.register_subclass("rdt")
@dataclass
class RDTControlConfig(ControlConfig):
    # Dataset identifier. By convention it should match '{hf_username}/{dataset_name}' (e.g. `lerobot/test`).
    repo_id: str
    # A short but accurate description of the task performed during the recording (e.g. "Pick the Lego block and drop it in the box on the right.")
    single_task: str
...
@ControlConfig.register_subclass("simoperate")
@dataclass
class SimoperateControlConfig(ControlConfig):
    # Limit the maximum frames per second. By default, no limit.
    fps: int | None = None
    teleop_time_s: float | None = None
    # Display all cameras on screen
